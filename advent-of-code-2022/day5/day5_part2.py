...
for line in f.readlines():
    ...
    length = len(stack_dict[from_stack])
    crates = stack_dict[from_stack][length - count:]
    stack_dict[from_stack] = stack_dict[from_stack][:length - count]
    stack_dict[to_stack] = stack_dict[to_stack] + crates

res = ""

for i in range(1, len(stack_dict)+1):
    res += stack_dict[str(i)][-1]
